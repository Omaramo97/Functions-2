...
const filerMixedNumbers= mixedNumbers.filter(function(element){
  return element % 2 === 0
})

const filterMixedNumbers= mixedNumbers.filter(element => element % 2 === 0)
...
const mappedPrices = prices.map(function(element){
  return element* 1.07
})

const mappedPricesArrow = prices.map(num => num * 1.07)
...
const reducePopulations = populations.reduce(function(acc, curr){
  return acc + curr
  })
  
  const reducePopulationsArrow = populations.reduce((acc, curr) => acc + curr)
...
const monstersInYourPocketFilter = monstersInYourPocket.filter(function(element){
  return element["CP"] > 200
})


const monstersInYourPocketFilterArrow = monstersInYourPocket.filter(element => element["CP"] > 200)
...
const ordersArray = orders.map(function(num){
...
const bobPurchases = purchases.filter(element => {
